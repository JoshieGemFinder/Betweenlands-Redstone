...
public class BlockScabystPistonMoving extends BlockContainer implements IScabystBlock {
	
    public static final PropertyDirection FACING = BlockScabystPistonExtension.FACING;
    public static final PropertyEnum<BlockScabystPistonExtension.EnumPistonType> TYPE = BlockScabystPistonExtension.TYPE;

    public BlockScabystPistonMoving(String name)
    {
        super(Material.PISTON);
        this.setDefaultState(this.blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH).withProperty(TYPE, BlockScabystPistonExtension.EnumPistonType.DEFAULT));
	...
	public void dropBlockAsItemWithChance(World worldIn, BlockPos pos, IBlockState state, float chance, int fortune)
    {
        ...
        {
            TileEntityScabystPiston tileentitypiston = this.getTilePistonAt(worldIn, pos);
    ...
    {
        TileEntityScabystPiston tileentitypiston = this.getTilePistonAt(source, pos);
        return tileentitypiston != null ? tileentitypiston.getAABB(source, pos) : FULL_BLOCK_AABB;
    }

    @Nullable
    private TileEntityScabystPiston getTilePistonAt(IBlockAccess iBlockAccessIn, BlockPos blockPosIn)
    {
        TileEntity tileentity = iBlockAccessIn.getTileEntity(blockPosIn);
        return tileentity instanceof TileEntityScabystPiston ? (TileEntityScabystPiston)tileentity : null;
    }
    ...
    {
        return this.getDefaultState().withProperty(FACING, BlockScabystPistonExtension.getFacing(meta)).withProperty(TYPE, (meta & 8) > 0 ? BlockScabystPistonExtension.EnumPistonType.STICKY : BlockScabystPistonExtension.EnumPistonType.DEFAULT);
    }
    ...
    public int getMetaFromState(IBlockState state)
    {
        ...
        if (state.getValue(TYPE) == BlockScabystPistonExtension.EnumPistonType.STICKY)
        {
            i |= 8;
        }
    ...
    {
        TileEntityScabystPiston tileentitypiston = this.getTilePistonAt(world, pos);
        if (tileentitypiston != null)
        {

...
public class BlockScabystDropper extends BlockDropper implements IModelInterface, IHidingDispenser {
	
	public BlockScabystDropper(String name) {
		super();
	...
	public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos,
			EntityPlayer player) {
		return new ItemStack(ModItems.SCABYST_DROPPER);
	}
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		return ModItems.SCABYST_DROPPER;
	}
    ...
    {
		TileEntityScabystDropper tile = new TileEntityScabystDropper();
        return tile;
    }
	
	@Override
	protected BlockStateContainer createBlockState() {
		return BlockStateContainerHelper.extendBlockstateContainer(super.createBlockState(), BlockScabystDispenser.HIDDEN);
	}
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		...
		return actualState.withProperty(BlockScabystDispenser.HIDDEN, this.shouldDispenserBeHidden(actualState, worldIn, pos));
	}
	...
	public void updateHidingState(IBlockState state, World worldIn, BlockPos pos) {
		...
		boolean alreadyHidden = state.getValue(BlockScabystDispenser.HIDDEN);
		boolean shouldBeHidden = this.shouldDispenserBeHidden(state, worldIn, pos);
		
		if(alreadyHidden != shouldBeHidden) {
			worldIn.setBlockState(pos, state.withProperty(BlockScabystDispenser.HIDDEN, shouldBeHidden), 2 | 16);

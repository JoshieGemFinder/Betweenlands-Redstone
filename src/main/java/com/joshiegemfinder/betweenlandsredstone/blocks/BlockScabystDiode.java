...
public abstract class BlockScabystDiode extends BlockHorizontal implements IScabystBlock {

	protected static final AxisAlignedBB REDSTONE_DIODE_AABB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 0.125D, 1.0D);
    protected final boolean isRepeaterPowered;

    protected BlockScabystDiode(boolean powered)
    {
    ...
    {
        return getScabystWeakPower(blockState, blockAccess, pos, side);
    }
    ...
    protected int calculateInputStrength(World worldIn, BlockPos pos, IBlockState state)
    {
        ...
        int i = ScabystWorldWrapper.getPower(worldIn, blockpos, enumfacing);
        ...
        else
        {
            ...
            int power = 0;
            //TODO Add any extra blocks in more else if statements
            /*if(block == Blocks.REDSTONE_WIRE) {
            	power = (Integer)iblockstate.getValue(BlockRedstoneWire.POWER);
            }
            else */if(block == ModBlocks.SCABYST_WIRE) {
            		power = (Integer)iblockstate.getValue(BlockScabystWire.POWER);
            }
            else {
        		power = 0;
            }
    ...
    protected int getPowerOnSide(IBlockAccess worldIn, BlockPos pos, EnumFacing side)
    {
        ...
        if (this.isAlternateInput(iblockstate))
        {
            ...
            {
                return (//TODO add more blocks
                		//block == Blocks.REDSTONE_WIRE ||
                		block == ModBlocks.SCABYST_WIRE
                		) ? ((Integer)iblockstate.getValue(BlockRedstoneWire.POWER)).intValue() : ScabystWorldWrapper.getStrongScabystPower(worldIn, pos, side);
            }
        }
        else
        {

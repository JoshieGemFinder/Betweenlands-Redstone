...
public class EntityScabystMinecartHopper extends EntityScabystMinecartContainer implements IHopper {

    private boolean isBlocked = true;
    private int transferTicker = -1;
    private final BlockPos lastPosition = BlockPos.ORIGIN;

    public EntityScabystMinecartHopper(World worldIn)
    {
        super(worldIn);
    }

    public EntityScabystMinecartHopper(World worldIn, double x, double y, double z)
    {
        super(worldIn, x, y, z);
    }
    ...
    {
        return BlockRegistry.SYRMORITE_HOPPER.getDefaultState();
    }
    ...
    {
        if (TileEntityHopperBetweenlands.pullItems(this))
        {
            return true;
        }
        else
        {
            ...
            if (!list.isEmpty())
            {
                TileEntityHopperBetweenlands.putDropInInventoryAllSlots((IInventory)null, this, list.get(0));
            }
    ...
    public void killMinecart(DamageSource source)
    {
        ...
        {
            this.dropItemWithOffset(Item.getItemFromBlock(BlockRegistry.SYRMORITE_HOPPER), 1, 0.0F);
        }
    }
